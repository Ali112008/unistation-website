...
/* ═══════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════ */
interface SiteData {
  stats: { target: number; prefix: string; suffix: string; label: string }[];
  universities: {
    rank: number; name: string; nameAr: string; abbr: string; fee: string;
    type: string; description: string; features: string[]; highlight: boolean;
  }[];
  basicPackage: {
    title: string; badge: string; description: string; totalPrice: string;
    currency: string; priceNote: string; installments: { label: string; amount: string; note: string }[];
    services: string[];
  };
  additionalPackage: {
    title: string; description: string; note: string; services: string[];
  };
  registration: {
...
/* universities and faqs now loaded from Sanity via useSiteData() */
        ...
        <div className="mt-20 grid grid-cols-2 md:grid-cols-4 gap-5 max-w-4xl mx-auto hero-animate hero-delay-5">
          {stats.map((s, i) => (
            <div key={i} className="bg-white/8 backdrop-blur-md border border-white/12 rounded-2xl p-5 hover:bg-white/15 transition-all duration-300 hover:-translate-y-1 hover:border-brand-gold/20">
              <AnimatedCounter target={s.target} prefix={s.prefix} suffix={s.suffix} duration={2500} delay={i * 200} />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <Card className="border-2 border-brand-gold/30 rounded-2xl shadow-2xl relative pt-4 card-shine-effect card-3d-tilt">
              ...
              <CardContent className="p-10">
                ...
                <div className="space-y-4 mb-8">
                  {basicPackage.installments.map((inst, idx) => (
        ...
        <FadeIn delay={200} className="mt-24">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-5">
            {additionalPackage.services.map((s, i) => (
              <FadeIn key={i} delay={i * 60}>
...
function CTASection() {
  return (
    <section className="py-24 sm:py-32 bg-gradient-to-br from-brand-purple-dark via-brand-purple to-brand-purple-light relative overflow-hidden animate-gradient-bg">
      <div className="absolute inset-0 bg-[url('/georgia-nature.png')] bg-cover bg-center opacity-10" />
