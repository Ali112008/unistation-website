...
export function Footer({ data }: { data?: SiteLayoutData }) {
  const brand = data?.brand || fallbackConfig.brand;
  const social = data?.social || fallbackConfig.social;
  const offices = data?.offices || fallbackConfig.offices;
  const topDestinations = data?.topDestinations || fallbackConfig.topDestinations;
  const packages = data?.packages || fallbackConfig.packages;
  const officeAddress = offices?.[0]?.address || fallbackConfig.offices[0]?.address || "Digital Park, A1, Dubai Silicon Oasis, Dubai";
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          ...
          <div className="lg:col-span-1">
            ...
            <div className="space-y-3">
              <a href={`tel:+${brand.whatsapp}`} className="flex items-center gap-3 text-gray-400 hover:text-white text-sm transition-colors">
            ...
            <div className="flex items-center gap-3 mt-6">
              {[
                ...
                { label: "TikTok", href: social.tiktok, icon: (
                ...
                { label: "YouTube", href: social.youtube, icon: (
      ...
      <div className="border-t border-white/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-6">
            <a href={`tel:+${brand.whatsapp}`} className="text-gray-500 hover:text-gray-300 text-sm transition-colors">
              +{brand.whatsapp}
            </a>
            <a
